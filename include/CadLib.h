...
public:
	...
	BOOL Create( LPCTSTR filename, bool overwrite=false );
	BOOL Close( );
	...
	// ENTITIES section
	BOOL Line( double x1, double y1, double z1, double x2, double y2, double z2);
	BOOL Circle( double cx, double cy, double cz, double r );
	BOOL Solid( int PointsNum, PREALPOINT Points );
	BOOL Text( LPCTSTR Text, double x1, double y1, double x2, double y2, double z, double Height, DWORD Justification, double Rotation=0, double WidthFactor=0, LPCTSTR StyleName=NULL );
	BOOL Text( LPCTSTR Text, double x, double y, double z, double Height, double Rotation=0, double WidthFactor=0, LPCTSTR StyleName=NULL );
	BOOL DimLinear( double x1, double y1, double x2, double y2, double x3, double y3, double angle, LPCTSTR text=NULL );
	BOOL Arc( double cx, double cy, double cz, double r, double StartAngle, double EndAngle, double Thickness=0.0 );
...
//	BOOL ChangeBlock( OBJHANDLE BlockObjhandle, PBLOCKHEADER pBlockHeader );
	...
	OBJHANDLE FindBlockBySonEntity(OBJHANDLE EntityHandle);
	...
	OBJHANDLE FindEntity( DWORD dwFindType, LPCTSTR strBlockName );
	void GetCurEntityHeader(ENTITYHEADER *header);
	void GetCurEntityData(LPVOID pData);
	OBJHANDLE Arc( double cx, double cy, double cz, double r, double StartAngle, double EndAngle );
	OBJHANDLE Circle( double cx, double cy, double cz, double r );
//	OBJHANDLE Ellipse( double cx, double cy, double ex, double ey, double Ratio, double StartParam=0.0, double EndParam=2*PI );
	...
	OBJHANDLE InsertBlock( OBJHANDLE BlockObjhandle, double x, double y, double z, double xScale=1, double yScale=1, double Rotation=0 );
	OBJHANDLE InsertBlock( LPCTSTR BlockName, double x, double y, double z, double xScale=1, double yScale=1, double Rotation=0 );
	OBJHANDLE Line( double x1, double y1, double z1, double x2, double y2, double z2);
	OBJHANDLE Solid( REALPOINT &Point0, REALPOINT &Point1, REALPOINT &Point2, REALPOINT &Point3 );
	OBJHANDLE Text( LPCTSTR Text, double x1, double y1, double x2, double y2, double z, double Height, short Justification, double Rotation=0, double WidthFactor=0 );
	OBJHANDLE Text( LPCTSTR Text, double x, double y, double z, double Height, double Rotation=0, double WidthFactor=0 );
	OBJHANDLE PolyLine( PENTVERTEX pVertex, int nVertex, unsigned short Flag=0 );
	OBJHANDLE Point(double x, double y, double z);
	...
	//Access
	PDRAWING GetDrawingData();
	void ReleaseDrawingData();
